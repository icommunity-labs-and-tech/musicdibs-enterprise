  ...
  return (
    <div className="flex items-center gap-4 py-3 border-b border-sand-50 dark:border-night-700/50 last:border-0">
      <div className={cn(
        ...
        <i className={cn(`ti ${icon} text-lg`, connected ? 'text-teal-500' : 'text-sand-900/30 dark:text-night-50/30')} />
      </div>
...
const PLANS = [
  {
    ...
    features: ['5 campañas / mes', '10.000 contactos', '1 usuario', 'KIE.ai + Suno', 'Analytics básico', 'Soporte email'],
    highlighted: false,
  },
  {
    ...
    features: ['25 campañas / mes', '100.000 contactos', '5 usuarios', 'Multi-vertical', 'Analytics avanzado', 'Integraciones CRM', 'Soporte prioritario'],
    highlighted: true,
  },
  {
    ...
    annualPrice: 19990,
    priceIds: {
      monthly: 'price_1TbQ0VFULeu7PzK6EaZQXHQK',
      annual:  'price_1TbQ0cFULeu7PzK6FcYlj2v2',
    },
    features: ['Campañas ilimitadas', 'Contactos ilimitados', 'Usuarios ilimitados', 'API access', 'SLA dedicado', 'Onboarding personalizado', 'Gestor de cuenta'],
    highlighted: false,
  },
]

const ANNUAL_PRICE_IDS = ['price_1TbQ0DFULeu7PzK63w9dujNN', 'price_1TbQ0PFULeu7PzK6z1MhbjAT', 'price_1TbQ0cFULeu7PzK6FcYlj2v2']
...
function BillingSection({ tenant }: { tenant: NonNullable<ReturnType<typeof useAuth>['tenant']> }) {
  ...
  const hasActiveSub = !!tenant.stripe_subscription_id && (stripeStatus === 'active' || stripeStatus === 'trialing')
  ...
  async function callBillingSession(action: 'checkout' | 'portal', priceId?: string) {
    ...
    try {
      ...
      alert(`Error al iniciar el pago: ${err?.message ?? 'Inténtalo de nuevo.'}`)
    } finally {
      setLoadingPlan(null)
    }
  }
    ...
    <div className="space-y-5">
          ...
          <div>
            ...
            <p className="text-xs text-sand-900/40 dark:text-night-50/40 mt-0.5">
              {tenant.stripe_price_id
                ? ANNUAL_PRICE_IDS.includes(tenant.stripe_price_id) ? 'Facturación anual' : 'Facturación mensual'
      ...
      <div className="flex items-center justify-center gap-3">
        ...
        <button
          ...
          className={cn('relative w-11 h-6 rounded-full transition-colors', billing === 'annual' ? 'bg-gold-400' : 'bg-sand-200 dark:bg-night-600')}
        >
          <div className={cn('absolute top-1 w-4 h-4 rounded-full bg-white shadow-sm transition-transform', billing === 'annual' ? 'translate-x-6' : 'translate-x-1')} />
        </button>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                plan.highlighted ? 'border-gold-400 bg-gold-50/50 dark:bg-gold-900/5' : 'border-sand-200 dark:border-night-600 bg-white dark:bg-night-800',
            ...
            >
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  <div className={cn('w-9 h-9 rounded-xl flex items-center justify-center shrink-0 mt-0.5', plan.highlighted ? 'bg-gold-100 dark:bg-gold-900/20' : 'bg-sand-100 dark:bg-night-700')}>
                  ...
                  <div>
                    ...
                    <div className="flex items-baseline gap-1 mt-0.5">
                      ...
                      {billing === 'annual' && (
                        <span className="text-xs text-sand-900/30 dark:text-night-50/30 ml-1">· €{plan.annualPrice}/año</span>
                      )}
                    </div>
                  </div>
                </div>
                <button
                  ...
                  className={cn(
                    ...
                    isCurrent ? 'bg-sand-100 dark:bg-night-700 text-sand-900/30 dark:text-night-50/30 cursor-default'
                      : plan.highlighted ? 'bg-gold-400 hover:bg-gold-500 text-night-900'
                      : 'bg-sand-900 dark:bg-night-50 text-white dark:text-night-900 hover:opacity-90',
                    'disabled:opacity-50'
                  )}
                >
...
export function Settings() {
  ...
  const { data: settings, isLoading } = useQuery<TenantSettings>({
    ...
    staleTime: 30_000,
    queryFn: async () => {
      const { data, error } = await supabase.from('tenant_settings').select('*').eq('tenant_id', tenant!.id).single()
  ...
  const { data: profiles } = useQuery({
    ...
    staleTime: 60_000,
    queryFn: async () => {
      const { data, error } = await supabase.from('profiles').select('*').eq('tenant_id', tenant!.id).order('created_at', { ascending: true })
      if (error) throw error
      return data ?? []
    },
  })
  ...
  const saveOrg = useMutation({
    mutationFn: async () => {
      const [r1, r2] = await Promise.all([
        ...
        supabase.from('tenant_settings').upsert({ tenant_id: tenant!.id, website: website || null, support_email: supportEmail || null }, { onConflict: 'tenant_id' }),
      ])
      if (r1.error) throw r1.error
      if (r2.error) throw r2.error
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tenant_settings'] }); toast.success('Organización guardada') },
  ...
  const saveKeys = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tenant_settings'] }); toast.success('Claves guardadas') },
    onError: (err: any) => { toast.error('Error al guardar claves', err?.message) },
  })

  const toggleIntegration = async (key: keyof TenantSettings['integrations'], connected: boolean) => {
    const updated = { ...integrations, [key]: { ...(integrations[key] ?? {}), connected } }
    setIntegrations(updated)
    await supabase.from('tenant_settings').upsert({ tenant_id: tenant!.id, integrations: updated }, { onConflict: 'tenant_id' })
    qc.invalidateQueries({ queryKey: ['tenant_settings'] })
  }

  const ROLE_LABELS: Record<string, string> = { admin: 'Admin', manager: 'Manager', analyst: 'Analyst', owner: 'Owner', member: 'Member', viewer: 'Viewer' }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in pb-12">
      <div>
        <h1 className="font-display text-2xl text-sand-900 dark:text-night-50 font-semibold">Configuración</h1>
        <p className="text-sm text-sand-900/50 dark:text-night-50/50 mt-0.5">Gestiona tu organización, integraciones y equipo</p>
      </div>
      ...
      {billingResult === 'success' && (
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-teal-50 dark:bg-teal-900/10 border border-teal-200 dark:border-teal-800/40">
          ...
          <div>
            ...
            <p className="text-xs text-teal-600/70 dark:text-teal-400/70 mt-0.5">Tu plan ha sido actualizado. Los cambios ya están activos.</p>
          </div>
        </div>
      )}
      {billingResult === 'cancel' && (
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-sand-50 dark:bg-night-700/50 border border-sand-200 dark:border-night-600">
          ...
          <p className="text-sm text-sand-900/60 dark:text-night-50/60">Pago cancelado. Tu plan actual permanece sin cambios.</p>
        </div>
      )}
      ...
      <SectionCard title="AI Providers" description="Claves de API para generación de voz y música">
        <ApiKeyField label="KIE.ai" hint="Generación de voz con IA" icon="ti-microphone" value={kieKey} onChange={setKieKey} placeholder="kie_live_…" />
        <ApiKeyField label="Suno API" hint="Generación de música con IA" icon="ti-wave-sine" value={sunoKey} onChange={setSunoKey} placeholder="suno_…" />
        <div className="flex justify-end pt-3">
      ...
      <SectionCard title="Integraciones CRM & Canales">
        <IntegrationRow icon="ti-database" name="Salesforce CRM" description="Sincroniza contactos y actualiza registros tras cada campaña"
          connected={integrations.salesforce?.connected} onConnect={() => toggleIntegration('salesforce', true)} onDisconnect={() => toggleIntegration('salesforce', false)} />
        <IntegrationRow icon="ti-topology-ring" name="HubSpot" description="Importa listas de contactos y envía eventos de engagement"
          connected={integrations.hubspot?.connected} onConnect={() => toggleIntegration('hubspot', true)} onDisconnect={() => toggleIntegration('hubspot', false)} />
        <IntegrationRow icon="ti-brand-whatsapp" name="WhatsApp Business" description="Canal alternativo de entrega para campañas de audio"
          connected={integrations.whatsapp?.connected} comingSoon />
      </SectionCard>

      <SectionCard title="Equipo" description="Miembros con acceso a esta organización">
        <div className="divide-y divide-sand-50 dark:divide-night-700/50">
          {(profiles ?? []).map(p => (
            <div key={p.id} className="flex items-center gap-3 py-3">
              <div className="w-8 h-8 rounded-full bg-gold-100 dark:bg-gold-900/20 flex items-center justify-center shrink-0">
                {p.avatar_url
                  ...
                  : <span className="text-sm font-semibold text-gold-600 dark:text-gold-400">{(p.full_name ?? '?')[0].toUpperCase()}</span>
                }
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-sand-900 dark:text-night-50 truncate">
                  {p.full_name ?? '—'}
                  {p.id === user?.id && <span className="ml-2 text-[10px] text-sand-900/30 dark:text-night-50/30">(tú)</span>}
                </p>
              </div>
          ...
          {(profiles ?? []).length === 0 && <p className="text-sm text-sand-900/40 dark:text-night-50/40 py-3">Sin miembros cargados</p>}
        </div>
        <div className="pt-4">
          <button disabled className="flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-xl border border-dashed border-sand-300 dark:border-night-600 text-sand-900/40 dark:text-night-50/40 cursor-not-allowed">
      ...
      <SectionCard title="Zona de riesgo">
        <div className="flex items-center justify-between py-2">
          <div>
            ...
            <p className="text-xs text-sand-900/40 dark:text-night-50/40 mt-0.5">Elimina permanentemente todos los datos. Esta acción no se puede deshacer.</p>
          </div>
          <button disabled className="text-sm font-medium px-4 py-2 rounded-xl text-red-400 border border-red-200 dark:border-red-900/40 opacity-40 cursor-not-allowed">
            Eliminar
          </button>
        </div>
      </SectionCard>
